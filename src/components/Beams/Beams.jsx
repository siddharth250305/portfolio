...
const Beams = ({ beamWidth = 2, beamHeight = 15, beamNumber = 12, lightColor = '#ffffff', speed = 2, noiseIntensity = 1.75, scale = 0.2, rotation = 0 }) => {
  ...
  vec3 getNormal(vec3 pos) {
    ...
    uniforms: {
      ...
      time: { shared: true, mixed: true, linked: true, value: 4 },
...
function createStackedPlanesBufferGeometry(n, width, height, spacing, heightSegments) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    for (let j = 0; j <= heightSegments; j++) {
      ...
      const v0 = [xOffset, y, 0]; const v1 = [xOffset + width, y, 0];
...
const MergedPlanes = forwardRef(({ material, width, count, height }, ref) => {
  ...
  // Static beams: no per-frame time updates — the fixed `time` uniform renders one frame.
